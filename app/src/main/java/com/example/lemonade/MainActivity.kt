...
class MainActivity : AppCompatActivity() {
    ...
    private fun clickLemonImage() {
        ...
        val resultImageView: ImageView = findViewById(R.id.image_lemon_state)
        when(lemonadeState)
        {
            "select" -> {
                ...
                squeezeCount = 0
                resultImageView.setImageResource(R.drawable.lemon_squeeze)
                setViewElements()
            }
            ...
            "squeeze" -> {
                if(lemonSize == 0 ){
                    ...
                    lemonSize = -1
                    squeezeCount = 0
                    resultImageView.setImageResource(R.drawable.lemon_drink)
                    setViewElements()
                }
                squeezeCount += 1
                lemonSize -= 1
            ...
            "drink" -> {
                ...
                resultImageView.setImageResource(R.drawable.lemon_restart)
                setViewElements()
            }

            "restart" -> {
                lemonadeState = SELECT
                resultImageView.setImageResource(R.drawable.lemon_tree)
                setViewElements()
            }
        }
    ...
    private fun setViewElements() {
        ...
        {
            "select" -> {
                textAction.setText("Select a Lemon from the tree")
            }

            "squeeze" -> {
                textAction.setText("Squeeze the Lemon! (Long press)")
            }
            "drink" -> {
                textAction.setText("Your Lemonade is ready to drink!")
            }

            "restart" -> {
                textAction.setText("Tasty, isn't it? Wanna try it again?")
            }
        }
